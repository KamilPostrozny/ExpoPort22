/**
 * Settings: the one host — where it is, who we are on it, how a session starts there, where the
 * last upload went — plus the app's own answers, the type size, the schemes, the tmux comforts.
 * Everything here is non-secret: the seed and the pinned host keys live in SecureStore instead
 * (T5).
 *
...
export type Settings = {
  ...
  /** What the host was running at the last connect — the list Setup offers. Cached because Setup is
   *  where the choice is made and there is no connection there to ask over; refreshed on every
...
export function decode(raw: unknown): Settings {
  ...
  // A blob written while the app had a host list keeps its first entry: the per-host fields are
  // spread over the top-level ones rather than given a new `STORAGE_KEY`, so an upgrade keeps the
  // machine the user already had.
