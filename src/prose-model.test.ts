...
test('a node CLI is prose when its script path names the app', () => {
  ...
  expect(proseFor('node', ['node /usr/lib/node_modules/@anthropic-ai/claude-code/cli.js'])).toBe(
    true,
  );
  expect(proseFor('node', ['/usr/local/lib/node_modules/.bin/pi'])).toBe(true);
  expect(proseFor('node', ['node /home/u/.local/share/pi-coding-agent/dist/cli.js'])).toBe(true);
