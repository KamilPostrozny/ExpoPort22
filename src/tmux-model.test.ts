...
test("the required half is in the conf whether or not the comforts are", () => {
  for (const conf of [generateConf(true), generateConf(false)]) {
    ...
    for (const table of ["copy-mode ", "copy-mode-vi"]) {
      expect(conf).toMatch(
        new RegExp(
          `bind -T ${table.trim()}\\s+WheelUpPane\\s+send -N1 -X scroll-up`,
        ),
      );
      // Down cancels the mode at the bottom: plain `scroll-down` would leave momentum notches
      // trapped in a buffer that looks live but is not.
      expect(conf).toMatch(
        new RegExp(
          `bind -T ${table.trim()}\\s+WheelDownPane\\s+send -N1 -X scroll-down-and-cancel`,
        ),
      );
    }
    ...
    for (const [wheel, key] of [
      ...
      expect(conf).toMatch(
        new RegExp(`bind -n ${wheel}\\s+if -F .#\\{\\|\\|:`),
      );
      ...
      expect(conf).toContain(
        `#{||:#{alternate_on},#{m/r:^(git|less)$,#{pane_current_command}}}' 'send -N1 ${key}'`,
      );
    }
    // The last fallback is a dead end for DOWN only: UP opens the scrollback (tmux's own
    // default, the designed way in), but DOWN must not re-enter copy mode — the leftover
    // momentum notches would trap the wheel with no key to escape it.
    expect(conf).toMatch(/WheelUpPane[^\n]*copy-mode -e/);
    expect(conf).not.toMatch(/WheelDownPane[^\n]*copy-mode/);
...
test("sessions: names only, tmux’s own chatter dropped", () => {
  ...
  expect(
    parseSessions("no server running on /tmp/tmux-1000/default\n"),
  ).toEqual([]);
...
test("file reads come back empty rather than failing on a missing file", () => {
  expect(readFileCommand(`~/${CONF_PATH}`)).toBe(
    `cat ~/${CONF_PATH} 2>/dev/null; true`,
  );
});
...
test("list-windows output parses, name last so separators in names shift nothing", () => {
  const output = [
    ...
    line([
      ...
      "120",
      "vim",
      `weird${SEP}name: with everything\t`,
    ]),
  ].join("\n");
  expect(parseWindows(output)).toEqual([
    ...
    // The name survives with the separator inside it — it is the tail, rejoined.
    {
      ...
      width: 120,
      command: "vim",
      name: `weird${SEP}name: with everything\t`,
    },
  ]);
});
...
test("the list has exactly as many windows as tmux printed — no phantom row", () => {
  ...
  const output =
    rows
      .map(([id, i]) =>
        line([id, String(i), "0", "/home/kamil", "80", "fish", "w"]),
      )
      .join("\n") + "\n";
...
test("lines that are not windows (tmux diagnostics, junk) never become cards", () => {
  ...
  expect(
    parseWindows(line(["not-an-id", "1", "0", "/", "80", "sh", "x"])),
  ).toEqual([]);
...
test("window commands target by tmux window id, never by index", () => {
  ...
  expect(newWindowCommand("port22")).toBe(
    "tmux new-window -a -t '=port22:{end}'",
  );
...
test("no window command can be built without a session to scope it to", () => {
  ...
  for (const build of [listWindowsCommand, newWindowCommand])
...
test("sameWindows sees every field a card draws", () => {
  ...
  expect(sameWindows([win(), win({ id: "@2", active: false })], [win()])).toBe(
    false,
  );
  expect(sameWindows([win()], [win(), win({ id: "@2", active: false })])).toBe(
    false,
  );
