# ExpoPort22 — Implementation Plan (archived)

> **HISTORICAL — NOT CURRENT INSTRUCTIONS.** Superseded by [current behaviour](../current.md)
> and [issue triage](../issues.md). Old T/§ references are retained for investigation only.
> Task statuses, design references, commands, and corrections describe old checkouts. Do not
> retrieve deleted designs or execute this plan. Use [the device workflow](../ship.md).
