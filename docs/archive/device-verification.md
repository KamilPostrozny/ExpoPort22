# Device verification cases and runs (archived)

> **HISTORICAL — NOT CURRENT ACCEPTANCE CRITERIA.** Use [the current test index](../../TESTS.md)
> and [device workflow](../ship.md). Ticks below apply only to their recorded builds. Old
> expectations, user-only screenshot instructions, and host setup commands may be wrong or unsafe.
> Do not replay them or bulk-read this execution diary as a current specification.
...
- Historical mismatch setup restricted: change the key only in a throwaway sshd on a separate
  port after pinning that test endpoint. Never change the real host key or system sshd config.
- Steps: Connect.
...
- Setup: connected.
- Historical outage step retired: never kill the user's tmux server or real sshd. Use the
  current T3.7 case with a disposable sshd/connection, then restore it and tap Reconnect.
