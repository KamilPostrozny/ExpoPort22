...
| T7.17 | Raw mode (prose off): type a line, then HOLD the software-keyboard Backspace for several seconds; repeat with Delete on a hardware keyboard | One character dies per tap; after ~300ms of holding the deletion continues at the app's own ~25/s until the finger lifts — the held press delivers one keydown and the page owns the repeat (`src/raw-hold-model.ts`). Deletion stops at the release, on a blur, on a mode flip and on any other key; a held deletion never outlives 6s even if the release is unobserved. Prose mode's own backspace path is unchanged. Both platforms. |
